...
Prisma 元数据现在还会保存源码索引的生命周期状态，以及面向 MCP 高层查询的持久化代码分析工件。
...
8. 后台会异步构建源码索引，为更高层的 MCP 查询提供项目概览、依赖关系、类型、Mock 数据和上下文搜索能力。
...
| `pnpm backfill:source-indexes` | 为历史已发布且快照已就绪的版本补建源码索引 |
...
底层文件工具直接读取 `ready` 源码快照；高层源码理解工具依赖异步索引，在索引未完成时可能返回 `pending`、`indexing` 或 `failed` 状态。
...
如果版本早于源码索引功能上线，或者异步索引过程被中断，也可以执行：
